   ...
   allcategories().then((res: any) => {
      if (res.code == 200) {
         ...
         list.value.forEach(item => {
            if (item.name === '去雾算法') {
               item.path = '/defoggingAlgorithm'
            } else if (item.name === '欺骗算法') {
               item.path = '/deceptionAlgorithm'
            } else if (item.name === '伪装算法') {
               item.path = '/camouflageAlgorithm'
            } else if (item.name === '局部路径规划') {
               item.path = '/localPlanning'
            } else if (item.name === '全局路径规划') {
               item.path = '/globalPlanning'
            }
         });
